...
/// Minimal blocking mutex. Zig 0.16 removed the old `std.Thread.Mutex` (the new
/// `std.Io.Mutex` needs an `Io`), but Jolt's custom allocator only needs a short
/// lock around its allocation map, so a spinlock is plenty.
const SpinMutex = struct {
    flag: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    pub fn lock(m: *SpinMutex) void {
        while (m.flag.swap(true, .acquire)) std.atomic.spinLoopHint();
    }
    pub fn unlock(m: *SpinMutex) void {
        m.flag.store(false, .release);
    }
};
...
pub const GlobalState = struct {
    ...
    mem_mutex: SpinMutex = .{}, // 0.16: std.Thread.Mutex removed; tiny atomic lock guards the alloc map
...
fn zphysicsFree(maybe_ptr: ?*anyopaque) callconv(.c) void {
    if (maybe_ptr) |ptr| {
        ...
        const mem = @as([*]u8, @ptrCast(ptr))[0..@as(usize, @intCast(info.size))]; // u48 size -> usize (u32 on wasm)
